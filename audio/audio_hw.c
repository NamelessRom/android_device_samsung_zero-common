...
static void select_devices(struct audio_device *adev)
{
    ...
    if (adev->mode == AUDIO_MODE_IN_CALL) {
        if (adev->input_gain_path && !strcmp(adev->input_gain_path, "none"))
            audio_route_apply_path(adev->ar, adev->input_gain_path);
        if (adev->output_gain_path && !strcmp(adev->output_gain_path, "none"))
            audio_route_apply_path(adev->ar, adev->output_gain_path);

        ALOGV("select_devices() input_gain(%s) output_gain(%s)", adev->input_gain_path, adev->output_gain_path);
    }
...
static void start_call(struct audio_device *adev)
{
    ...
    ALOGV("%s: input_gain(%s) output_gain(%s)", __func__, adev->input_gain_path, adev->output_gain_path);
